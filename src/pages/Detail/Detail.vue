          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="attr in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  changepirce="0"
                  v-for="attrVal in attr.spuSaleAttrValueList"
                  :key="attrVal.id"
                  :class="{ active: attrVal.isChecked == 1 }"
                  @click="ChangeVal(attrVal, attr.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  class="itxt"
                  @input="handler"
                  @blur="goodSNum = goodSNum == '' ? 1 : goodSNum"
                  :value="goodSNum"
                />
...
export default {
  ...
  methods: {
    //切换属性高亮
    ChangeVal(clickVal, arrVal) {
      arrVal.forEach((item) => {
        item.isChecked = 0;
      });
      clickVal.isChecked = 1;
    },
    handler(event) {
      event.target.value = event.target.value.replace(/[^\d]/g, "");
      this.goodSNum = event.target.value;
    },
    addShopCart() {
      this.$store
        ...
        .then((res) => {
          ...
          //整理商品数据
          const project = {};
          project.id = this.skuInfo.id;
          //获取商品参数
          let attr = [];
          for (let i = 0; i < this.spuSaleAttrList.length; i++) {
            let itemAttr = {};
            let attrval = this.spuSaleAttrList[i];
            project[attrval.saleAttrName] = this.spuSaleAttrList.saleAttrName;
            for (let j = 0; j < attrval.spuSaleAttrValueList.length; j++) {
              if (attrval.spuSaleAttrValueList[j].isChecked == 1) {
                itemAttr[attrval.saleAttrName] =
                  attrval.spuSaleAttrValueList[j].saleAttrValueName;
              }
            }
            attr.push(itemAttr);
          }
          project["attr"] = attr;
          //获取加购得数量
          project.addCartNum = this.goodSNum;
          //获取商品名字
          project.skuName = this.skuInfo.skuName;
          //商品图片
          project.img = this.skuInfo.skuDefaultImg;
          this.$store.dispatch("getProject", project);
        })
        .catch((err) => {});
    },
  },

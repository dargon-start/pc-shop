  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <ul
          class="list clearfix"
          v-for="order in tradeInfo.detailArrayList"
          :key="order.skuId"
        >
          <li>
            <img :src="order.imgUrl" alt="" style="width: 50px; height: 60px" />
          </li>
          <li>
            <p class="skuName">
              {{ order.skuName }}
...
<script>
import { getaddress, getTrade } from "@/network/trade";
import { submitOrder } from "@/network/pay";
export default {
  name: "Trade",
  data() {
    return {
  ...
  methods: {
    async submitOrder() {
      let data = {
        ...
        paymentWay: "ONLINE",
        orderComment: this.liveMsg,
        orderDetailList: this.tradeInfo.detailArrayList,
      };
      const res = await submitOrder(this.tradeInfo.tradeNo, data);
      if (res.data.code == 200) {
        this.$router.push(`/pay?orderId=${res.data.data}`);
      }
      {
